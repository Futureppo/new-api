...
func setupProbeGuardTest(t *testing.T, dryRun bool) {
	...
	require.NoError(t, db.AutoMigrate(&model.User{}, &model.IPBan{}, &model.ProbeAbuseState{}, &model.Log{}))
	model.DB = db
	model.LOG_DB = db
...
func TestProbeGuardDryRunRecordsWithoutPenalties(t *testing.T) {
	...
	var state model.ProbeAbuseState
	err := model.DB.Where("user_id = ?", 1001).First(&state).Error
...
func TestProbeGuardEscalatesTemporaryThenPermanentPenalty(t *testing.T) {
	...
	state, err := model.GetProbeAbuseState(1002)
	require.NoError(t, err)
	...
	resetProbeGuardMemoryForTest()
	secondErr := fireProbeGuardBurst(t, 1002, "8.8.4.4")
	...
	ban, err = model.GetIPBanByTarget("8.8.4.4")
	require.NoError(t, err)
	require.Greater(t, ban.ExpiresAt, common.GetTimestamp()+3000)

	resetProbeGuardMemoryForTest()
	thirdErr := fireProbeGuardBurst(t, 1002, "1.1.1.1")
	...
	ban, err = model.GetIPBanByTarget("1.1.1.1")
	require.NoError(t, err)
...
func TestProbeGuardIgnoresRepeatedSingleModel(t *testing.T) {
	...
	var state model.ProbeAbuseState
	err := model.DB.Where("user_id = ?", 1003).First(&state).Error
	require.True(t, errors.Is(err, gorm.ErrRecordNotFound))
}
